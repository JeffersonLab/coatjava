...
public class MUCALCluster extends ArrayList<MUCALHit> {
	...
	public double getEnergy() {
        ...
        public double getFullEnergy(IndexedTable energyTable) {
            ...
            int seedID = MUCALHit.REFCOMPONENT;//this.get(0).get_COMPONENT();
...
//				double wi = hit_in_clus.get_Edep();    // de-comment for arithmetic weighting
	...
	public void setStatus(IndexedTable clusterTable) {
        ...
        private double weight(MUCALHit hit, double clusEnergy) {
            return Math.max(0., (3.45+Math.log(hit.get_Edep()/clusEnergy)));
        }
        
        public boolean containsHit(MUCALHit hit, IndexedTable thresholds, IndexedTable clusterTable) {
            boolean addFlag = false;
            if(hit.get_Edep()>thresholds.getDoubleValue("thresholdCluster",1,1,MUCALHit.REFCOMPONENT)) {
