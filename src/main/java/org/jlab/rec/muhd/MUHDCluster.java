...
public class MUHDCluster extends ArrayList<MUHDHit> {
	...
	public double getLayerMultiplicity(int ilayer) {
        ...
        public boolean isgoodCluster() {
            if(this.getSize()  > MUHDConstants.CLUSTER_MIN_SIZE   &&
               this.getEnergy()> MUHDConstants.CLUSTER_MIN_ENERGY && 
...
//                System.out.println("DT: " + tDiff + "(" + MUHDConstantsLoader.time_window 
//                             + ")\t DX: " + xDiff + "(" + MUHDConstantsLoader.hit_distance 
//                             + ")\t DY: " + yDiff + "(" + MUHDConstantsLoader.hit_distance + ")");
                if(tDiff <= MUHDConstants.TIME_WINDOW && 
                   xDiff <= MUHDConstants.HIT_DISTANCE && 
                   yDiff <= MUHDConstants.HIT_DISTANCE) addFlag = true;
            }
            return addFlag;
        }
	...
	public static  ArrayList<MUHDCluster> getSignals(EvioDataBank bank) {
	     ...
	     ArrayList<MUHDCluster> signals = new ArrayList<MUHDCluster>();
	      
//	     for(int i = 0; i<size; i++){  
//	    	 MUHDCluster signal = new MUHDCluster(signalID[i]);	
