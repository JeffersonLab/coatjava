...
public class MUHDHit implements Comparable<MUHDHit>{
	// class implements Comparable interface to allow for sorting a collection of hits by Edep values
	
	// constructor 
	public MUHDHit(int i, int Sector, int Layer, int ID, int ADC, int TDC, IndexedTable charge2Energy, IndexedTable timeOffsets, IndexedTable geometry) {
		this._Sector = Sector;
		this._Layer = Layer;
		this._ID    = ID;
		this._ADC   = ADC;
		...
		this.set_Edep(((double) this._ADC)*MUHDConstants.FADC_TO_CHARGE
				                  *charge2Energy.getDoubleValue("mips_energy", Sector, Layer, ID)
			                          /charge2Energy.getDoubleValue("mips_charge", Sector, Layer, ID));
		this.set_Time(((double) this._TDC)/MUHDConstants.TIMECONVFAC
...
//		System.out.println(this._Dx + " " + this._Dy);
	...
	public MUHDHit(int i, int Sector, int Layer, int ID, int ADC, float time, IndexedTable charge2Energy, IndexedTable timeOffsets, IndexedTable geometry) {
		this._Sector = Sector;
		this._Layer = Layer;
		this._ID    = ID;
		this._ADC   = ADC;
...
//	public static  ArrayList<MUHDHit> getRawHits(EvioDataBank bank) {
//
...
//	     int size = id.length;
//
//	     ArrayList<MUHDHit> hits = new ArrayList<MUHDHit>();
//	      
//	      for(int i = 0; i<size; i++){
//	  
//	          if(adc[i]!=-1 && tdc[i]!=-1 ){
//	        	 MUHDHit hit = new MUHDHit(i,sector[i], layer[i], id[i], adc[i], tdc[i]);
//	             hits.add(hit); 
//	          }
//	          
//	      }
...
//	}
	...
	public static boolean passHitSelection(MUHDHit hit) {
		// a selection cut to pass the hit. 
		if(hit.get_Edep() > MUHDConstants.EN_THRES) {
			return true;
		} else {
			return false;
		}		
	...
	@Override
	public int compareTo(MUHDHit arg0) {
