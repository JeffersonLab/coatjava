...
public class RICHCalibration{
    ...
    private ArrayList<IndexedTable> pixelTables     = new ArrayList<>();
    ...
    public void init_CalConstantsTxT(int ifile, int isec, int ncalls){
        ...
        if(ifile==1){
            ...
            *  TIME_WALKs
            */
            String walk_filename = "calibration/rich/richModule1/time_walk.txt";
    ...
    public void dump_AeroConstants(int irich) {
        ...
        for (int ila=0; ila<4; ila++){
            for(int ico=0; ico<RICHGeoConstants.NAERCO[ila]; ico++){
                if(ico<2 || ico>RICHGeoConstants.NAERCO[ila]-3){
    ...
    public int get_AeroStatus(int isec, int ila, int ico, int iqua) {
        ...
        if(ico<0 || ico>=RICHGeoConstants.NAERCO[ila]) return 0;
        int irich = find_RICHModule(isec);
        if(irich==0)return 0;
