...
public class TrackSeederBMTLinker {
    ...
    public TrackSeederBMTLinker(Swim swimmer, double xb, double yb) {
        float[] b = new float[3];
    ...
    public List<Seed> findSeed(List<Cross> bmt_crosses, List<Pixel> pixels) {
        ...
        List<ArrayList<Cross>> zrtracks = trseed1.getSeeds(bmtC_crosses,bmtcrs);
        //this.removeCompleteZROverlaps(zrtracks);
        if(Constants.getInstance().seedingDebugMode) {
            this.printListCrosses(zrtracks, "RZ Seeds");
        }
        List<Seed> cands = this.match2BMT(zrtracks, bmtcrs, pixels);
        if(Constants.getInstance().seedingDebugMode) {
            this.printListSeedCrosses(cands, "BMT Seeds");
        }
        for (Seed seed : cands) {
    ...
    private List<Seed> match2BMT(List<ArrayList<Cross>> zrtracks, 
            Map <Integer, Map <Integer, List<Cross>>> bmtcrs, List<Pixel> pixels) {
        ...
        for (List<Cross> zrcross : zrtracks) {  
            ...
            Pixel matchedPixel=null;
            double pixDoca = Double.POSITIVE_INFINITY;
            for(Pixel pix : pixels) {
                double Zref = m*Math.sqrt(pix.x*pix.x+pix.y*pix.y) + b;
                double doc = Math.abs(Zref-pix.z);
                if(doc<pixDoca) {
                    pixDoca=doc;
                    matchedPixel = pix;
                }
            ...
            if(matchedPixel!=null) {
                ...
                if(trseed2.passBMTCircle2Pix(pass, matchedPixel)) {
