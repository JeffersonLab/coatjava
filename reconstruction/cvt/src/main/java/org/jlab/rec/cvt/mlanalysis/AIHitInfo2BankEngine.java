 ...
 *
 */
public class AIHitInfo2BankEngine extends ReconstructionEngine {
    ...
    public AIHitInfo2BankEngine(String name) {
        super(name, "ziegler", "6.0");
    }

    public AIHitInfo2BankEngine() {
   ...
   public void registerBanks() {
    ...
    private List<AIObject> AIBankEntry(int evNb, org.jlab.rec.cvt.track.Track t, 
    ...
    private AIObject AIBankEntry(int evNb, Cluster c, 
            double mcPtk, double mcThetatk, double mcPhitk, double mcVxtk, double mcVytk, double mcVztk) {
    ...
    public boolean processDataEvent(DataEvent event) {
       ...
       if (event.hasBank("MC::Particle") == false) return false;
        ...
        double mcVx = (double) bank.getFloat("vx", 0);
        double mcVy = (double) bank.getFloat("vy", 0);
        ...
        if(tracks!=null) {
            ...
            for(org.jlab.rec.cvt.track.Track t : tracks) {
                ...
                List<AIObject> aio =this.AIBankEntry(ev, t, mcP, mcTheta, mcPhi, mcVx, mcVy, mcVz);
        ...
        for(Cluster c : useClusters) { 
            if(c.getAssociatedTrackID()==-1) {
                ...
                if(c.getSector()==aihs.svtTrkSectors[c.getRegion()-1] 
                    ...
                    AIObject aio =this.AIBankEntry(ev, c,  mcP, mcTheta, mcPhi, mcVx, mcVy, mcVz);
                    if(aio!=null)
                        aios.add(aio);
                }
            }
        ...
        DataBank aibank = event.createBank("cvtml::clusters", aios.size());   
        ...
        List<Hit> bmtHits = new ArrayList<>();
        
        for(Double d : selectedHits.keySet()) {
            for(Hit h : selectedHits.get(d).get(0)) {
                if(h.getDetector()==DetectorType.BST)
                    svtHits.add(h);
                if(h.getDetector()==DetectorType.BMT)
                    bmtHits.add(h);
            }
            for(Hit h : selectedHits.get(d).get(1)) {
                if(h.getDetector()==DetectorType.BST)
                    svtHits.add(h);
                if(h.getDetector()==DetectorType.BMT)
                    bmtHits.add(h);
        ...
        List<DataBank> banks = new ArrayList<>();
            if(!svtHits.isEmpty()) banks.add(RecoBankWriter.fillSVTHitBank(event, svtHits, this.getSvtHitBank()));
            ...
            if(!bmtHits.isEmpty()) banks.add(RecoBankWriter.fillBMTHitBank(event, bmtHits, this.getBmtHitBank()));
            ...
            if(trackFinder.getSVTclusters()!=null) {
                banks.add(RecoBankWriter.fillSVTClusterBank(event, trackFinder.getSVTclusters(), this.getSvtClusterBank()));
                banks.add(aibank);
            }
    ...
    public static void main(String[] args) {
        ...
        AIHitInfo2BankEngine en = new AIHitInfo2BankEngine();
        en.setVariation(var);
        en.fileName = "MLSample1_wHits.hipo";
        en.svtOnly=false;
        en.init();
        ...
        for(String inputFile :  inputList) {
            ...
            reader.getReader().getSchemaFactory().addSchema(en.schemaFactory.getSchema("cvtml::clusters"));
