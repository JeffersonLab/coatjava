...
public class SampleMaker extends ReconstructionEngine {
    ...
    private String mlHitBank;
    private String mlSeedBank;
    private String mlTrackBank;
    ...
    public void registerBanks() {
        ...
        this.setMlHitBank("CVT" + prefix + "::MLHit");
        this.setMlSeedBank("CVT" + prefix + "::MLSeed");
        this.setMlTrackBank("CVT" + prefix + "::MLTrack");
        ...
        super.registerOutputBank(this.mlSeedBank);
        super.registerOutputBank(this.mlTrackBank);
        super.registerOutputBank(this.mlHitBank);
    ...
    public boolean processDataEvent(DataEvent event) {
        ...
        List<ArrayList<Cluster>> clusters = reco.findClusters();
         for(int i = 0; i < clusters.size(); i++) {
            for(int j = 0; j < clusters.get(i).size(); j++) {
                double clsSize = clusters.get(i).get(j).size();
                double centroid = clusters.get(i).get(j).getCentroid();
                double seed = (double) clusters.get(i).get(j).getSeed().getStrip().getStrip();
                for(Hit h : clusters.get(i).get(j)) {
                    double strip = h.getStrip().getStrip();
                    double cweight = 1/(1+Math.abs(strip-centroid));
                    double sweight = 1/(1+Math.abs(strip-seed));
                    h.setCweight(cweight);
                    h.setSweight(sweight);
                }
            }
         }
        ...
        if(crosses != null) {
            if(Constants.getInstance().isCosmics) {
                ...
                List<StraightTrack>  seedsSt = trackFinder.getSeeds(event, clusters.get(0), clusters.get(1), crosses);
                List<StraightTrack> tracksSt = trackFinder.getTracks(event, this.isInitFromMc(), 
                                                                          this.isKfFilterOn(), 
                                                                          this.getKfIterations());
                if(seedsSt!=null) banks.add(RecoBankWriter.fillStraightSeedsBank(event, seedsSt, "CVTRec::CosmicSeeds"));
                if(tracksSt!=null) {
                    banks.add(RecoBankWriter.fillStraightTracksBank(event, tracksSt, "CVTRec::Cosmics"));
                    banks.add(RecoBankWriter.fillStraightTracksTrajectoryBank(event, tracksSt, "CVTRec::Trajectory"));
                    banks.add(RecoBankWriter.fillStraightTrackKFTrajectoryBank(event, tracksSt, "CVTRec::KFTrajectory"));
                }            
            } 
            else {
                ...
                seeds = trackFinder.getSeeds(clusters, crosses);
                ...
                if(seeds!=null) {
                    ...
                    for(Seed s : seeds) { 
                        for(Cluster c: s.getClusters()) {
                            for(Hit h : c) {
                                h.setAssociatedSeedID(s.getId());
                                h.setSeedBankRow(bankrow);
                            }
                        }
                        bankrow++;
                    }
    ...
    //                banks.add(RecoBankWriter.fillTrackCovMatBank(event, tracksSt, this.getCovMat()));
                    ...
                    for(Track t : tracks) { 
                        for(Cluster c: t.getSeed().getClusters()) {
                            for(Hit h : c) {
                                h.setAssociatedTrackID(t.getId());
                                h.setTrackBankRow(bankrow2);
                            }
                        }
                        bankrow2++;
                    }
        ...
        //ML banks
        TrackingPerformance.TrackingPerformance(event, hits, seeds, tracks);
        if(hits!=null) {
            banks.add(BankOutput.fillHitsBank(event, hits, this.getMlHitBank()));
        }
        if(seeds!=null) {
            banks.add(BankOutput.fillSeedsBank(event, seeds, this.getMlSeedBank()));
        }
        if(tracks!=null) {
            banks.add(BankOutput.fillTracksBank(event, tracks, this.getMlTrackBank()));
        }
    ...
    public void setMlHitBank(String mlHitBank) {
        this.mlHitBank = mlHitBank;
    }

    public void setMlSeedBank(String mlSeedBank) {
        this.mlSeedBank = mlSeedBank;
    }
    
    public void setMlTrackBank(String mlTrackBank) {
        this.mlTrackBank = mlTrackBank;
    }
    ...
    public String getMlHitBank() {
        return this.mlHitBank;
    }

    public String getMlSeedBank() {
        return this.mlSeedBank;
    }
    
    public String getMlTrackBank() {
        return this.mlTrackBank;
    }
