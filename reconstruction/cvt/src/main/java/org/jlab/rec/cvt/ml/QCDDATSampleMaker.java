...
public class QCDDATSampleMaker extends ReconstructionEngine {
    ...
    public boolean processDataEventUser(DataEvent event) {
        ...
        hitRead.fetch_BMTHits(event, swimmer, bmtStatus, bmtTime, bmtStripVoltage, bmtStripThreshold);

        List<ArrayList<Hit>>  hits = new ArrayList<>();
        hits.add(hitRead.getSVTHits() == null ? new ArrayList<>() : (ArrayList<Hit>) hitRead.getSVTHits());
        hits.add(hitRead.getBMTHits() == null ? new ArrayList<>() : (ArrayList<Hit>) hitRead.getBMTHits());
        ...
        for (int i = 0; i < hits.size(); i++) {
            for (int j = 0; j < hits.get(i).size(); j++) {
                ...
                var hit = hits.get(i).get(j);

                int layer = hit.getLayer();
                if(i>0) layer+=6;

                int mctrue = hit.MCstatus==0 ? 0 : 1;

                bank.setShort( "id",     index, (short) hit.getId()               );
                bank.setShort( "mctid",  index, (short) hit.getAssociateMCTrkId() );
                bank.setByte(  "sector", index, (byte)  hit.getSector()           );
                bank.setByte(  "layer",  index, (byte)  layer                     );
                bank.setShort( "strip",  index, (short) hit.getStrip().getStrip() );
                bank.setFloat( "energy", index, (short) hit.getStrip().getEdep()  );
                bank.setFloat( "time",   index, (short) hit.getStrip().getTime()  );
                bank.setByte(  "mctrue", index, (byte)  mctrue                    );

                if (  hit.getDetector()==DetectorType.BST ||
                     (hit.getDetector()==DetectorType.BMT && hit.getType()==BMTType.Z))
                {
                    Line3D sline = hit.getStrip().getLine();
                ...
                else if (hit.getDetector()==DetectorType.BMT && hit.getType()==BMTType.C) {
                    Arc3D sarc = hit.getStrip().getArc();
