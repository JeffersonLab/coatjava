...
public class DCTBPostHBTAI extends DCEngine {
	...
	IndexedTable beamOffset = this.getConstantsManager().getConstants(run, Constants.BEAMPOS);
        ...
        HBHitReader hitRead = new HBHitReader(this.getBanks(), this.getRawBankOrders(), super.getConstantsManager(), Constants.getInstance().dcDetector); //vz; modified reader to read regular or ai hits
        hitRead.read_AITrkgHBHits(event);
        List<FittedHit> hits = hitRead.getDCHits();
        ...
        Map<Integer, Track> trkMap = new HashMap<>();
        ...
        for(Integer it : segmentsMap.keySet()) {  
            ...
            if(trkMap.containsKey(it)) { 
                ...
                if(Math.abs(Swimmer.getTorScale()) < 0.001){
                    ...
                    List<Surface> measSurfaces = getMeasSurfaces(trk, Constants.getInstance().dcDetector);
                    StateVecs svs = new StateVecs();
                    org.jlab.clas.tracking.kalmanfilter.AStateVecs.StateVec initSV = svs.new StateVec(0);
                    getInitState(trk, measSurfaces.get(0).measPoint.z(), initSV, kFZRef, dcSwim, new float[3]);
                    kFZRef.initFromHB(measSurfaces, initSV, trk.get(0).get(0).get(0).get_Beta());
                    kFZRef.runFitter();
                    ...
                    if (kFZRef.setFitFailed==false && kFZRef.finalStateVec!=null) { 
                        ...
                        trk.set_P(1./Math.abs(kFZRef.finalStateVec.Q));
                        trk.set_Q((int)Math.signum(kFZRef.finalStateVec.Q));                

                        trkcandFinder.setTrackPars(trk, new Trajectory(), trjFind, fn, kFZRef.finalStateVec.z, Constants.getInstance().dcDetector, dcSwim, beamXoffset, beamYoffset);
                        // candidate parameters are set from the state vector
                        if (trk.fit_Successful == false) {
                            continue;
                        }                

                        trk.set_FitChi2(kFZRef.chi2);
                        trk.set_FitNDF(kFZRef.NDF);
                        trk.setStateVecs(kfStateVecsAlongTrajectory);
                        trk.set_FitConvergenceStatus(kFZRef.ConvStatus);
                        if (trk.get_Vtx0().toVector3D().mag() > 500) {
                            continue;
                        }

                        // get CovMat at vertex
                        Point3D VTCS = crosses.get(0).getCoordsInSector(trk.get_Vtx0().x(), trk.get_Vtx0().y(), trk.get_Vtx0().z());
                        trk.set_CovMat(kFZRef.propagateToVtx(crosses.get(0).get_Sector(), VTCS.z()));
                    ...
                    org.jlab.clas.tracking.kalmanfilter.AStateVecs.StateVec initSV = svs.new StateVec(0);
                    getInitState(trk, measSurfaces.get(0).measPoint.z(), initSV, kFZRef, dcSwim, new float[3]);
                    kFZRef.initFromHB(measSurfaces, initSV, trk.get(0).get(0).get(0).get_Beta(), useDAF);
                    ...
                    if (kFZRef.setFitFailed==false && kFZRef.finalStateVec!=null) { 
                        ...
                        trk.set_FitChi2(kFZRef.chi2);
                        trk.set_FitNDF(kFZRef.NDF);
                        trk.set_NDFDAF(kFZRef.getNDFDAF());
                        trk.setStateVecs(kfStateVecsAlongTrajectory);
                        trk.set_FitConvergenceStatus(kFZRef.ConvStatus);
                        if (trk.get_Vtx0().toVector3D().mag() > 500) {
                            continue;
