                            ...
                            for(int ibfield =0; ibfield<1; ibfield++) {
                                for (int tb = 250; tb< 300; tb++) {
                                    ...
                                    for(int icosalpha =0; icosalpha<maxBinIdxAlpha+1; icosalpha++) {
                                            ...
                                            double Xdoca=tde.interpolateOnGrid((double) Bf, Xalpha,0, Xtime, s, r);
                                                LOGGER.log(Level.FINE, "Bbin "+ibfield+" B "+ (float)Bf+" sl "+(r+1)+" time "+Xtime+" tb "+tb+" timeBin "+tde.getTimeIdx(Xtime)
                                                        +" icosalpha "+icosalpha+" Xalpha "+(float) Xalpha + " dis "+ (float)DISTFROMTIME[s][r][bbin][icosalpha][0][tde.getTimeIdx(Xtime)] +" time' "+
                                                      (float)  calc_Time( Xdoca,  Xalpha, Bf, s+1, r+1) +" tdix "+tde.getTimeIdx(calc_Time( Xdoca,  Xalpha, Bf, s+1, r+1)));
                                            //}
                                            }
        ...
        for(int s = 0; s<6; s++ ){ // loop over sectors
            for(int r = 0; r<6; r++ ){ //loop over slys
                ...
                distbeta[s][r] = t2dPressure.getDoubleValue("distbeta_a0", s+1,r+1,0)
                        +t2dPressure.getDoubleValue("distbeta_a1", s+1,r+1,0)*dp
                        +t2dPressure.getDoubleValue("distbeta_a2", s+1,r+1,0)*dp*dp;
