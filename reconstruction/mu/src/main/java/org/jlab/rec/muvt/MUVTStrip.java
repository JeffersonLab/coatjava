...
/**
 * MUVT strip, defined based on ADC bank information and 3D line provided 
...
public class MUVTStrip implements Comparable {
    ...
    public MUVTStrip(int sector, int layer, int component){
        this.desc.setSectorLayerComponent(sector, layer, component);
    }

    public MUVTStrip(int sector, int layer, int component, int ADC, int TDC){
        this.desc.setSectorLayerComponent(sector, layer, component);
        this.ADC = ADC;
        this.TDC = TDC;
    ...
    public boolean isNeighbour(MUVTStrip strip){
    ...
    public boolean isInTime(MUVTStrip strip) {
        return Math.abs(this.getTime() - strip.getTime()) < MUVTConstants.COINCTIME;
    }     
    
    @Override
    public int compareTo(Object o) {
        MUVTStrip ob = (MUVTStrip) o;
    ...
    public static List<MUVTStrip> getStrips(DataEvent event, URWellStripFactory factory, ConstantsManager ccdb) {
        ...
        if(event.hasBank("URWELL::adc")){
            ...
            for(int i = 0; i < bank.rows(); i++){
                ...
                MUVTStrip  strip = new MUVTStrip(sector,  layer,   comp); 
