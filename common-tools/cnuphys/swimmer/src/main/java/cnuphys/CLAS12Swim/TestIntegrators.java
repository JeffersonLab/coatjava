...
public class TestIntegrators {
	...
	private static void testBasicSwim(int n, long seed) {
		...
		for (CLAS12Swimmer swimmer : swimmers) {
			for (int i = 0; i < 5; i++) {
				result = swimmer.swim(rtd.q[i], rtd.xo[i], rtd.yo[i], rtd.zo[i], rtd.p[i], rtd.theta[i], rtd.phi[i],
						sMax, h, tolerance);
			}
	...
	private static void testSwimRho(int n, long seed) {
		...
		for (CLAS12Swimmer swimmer : swimmers) {
			for (int i = 0; i < 10; i++) {
				result = swimmer.swimRho(rtd.q[i], rtd.xo[i], rtd.yo[i], rtd.zo[i], rtd.p[i], rtd.theta[i], rtd.phi[i],
						rhoTarget[i], accuracy, sMax, h, tolerance);
				System.out.println("SOLVER: " + swimmer.getSolver().name() + "\n" + result);
			}
			// System.out.println("SOLVER: " + swimmer.getSolver().name() + "\n" + result);
		}
		...
		for (CLAS12Swimmer swimmer : swimmers) {
			...
			System.out.println("SOLVER: " + swimmer.getSolver().name() + "  good count: " + count + "  max del: "
					+ delMax + " cm");
		}
