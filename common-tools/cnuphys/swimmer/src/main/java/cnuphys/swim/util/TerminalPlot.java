...
public class TerminalPlot {
    ...
    public static void plot2D(int width, int height, String title, double x[],
	    double y[]) {
	    ...
	    printRow(aval, row, firstDataRow, lastDataRow, firstDataCol,
		    lastDataCol, rows, cols);
			blankLines(1);
		}
	...
	// print a row
    private static void printRow(double aval, int row, int firstDataRow,
	    int lastDataRow, int firstDataCol, int lastDataCol, int rows[],
	    int cols[]) {
	...
	public static void main(String arg[]) {
	    ...
	    y[i] = x[i] * Math.tan(theta) - g * x[i] * x[i]
		    / (2 * v * v * cost * cost);
		}
