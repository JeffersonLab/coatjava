...
public class Difeq {
    final int mm, n, mpt;
    final double h, c2, anorm;
    final double[] x;

    public Difeq(final int mmm, final int nn, final int mptt, final double hh,
	    final double cc2, final double anormm, final double[] xx) {
	mm = mmm;
	n = nn;
	mpt = mptt;
	h = hh;
    ...
    public void smatrix(final int k, final int k1, final int k2, final int jsf,
	    final int is1, final int isf, final int[] indexv,
	    final double[][] s, final double[][] y) {
	...
	if (k == k1) {
	    ...
	    s[2][3 + indexv[1]] = 0.0;
	    s[2][3 + indexv[2]] = 1.0;
	    s[0][jsf] = y[0][k] - y[0][k - 1] - 0.5 * h
		    * (y[1][k] + y[1][k - 1]);
	    s[1][jsf] = y[1][k]
		    - y[1][k - 1]
		    - temp
		    * ((x[k] + x[k - 1]) * 0.5 * (mm + 1.0)
			    * (y[1][k] + y[1][k - 1]) - temp2 * 0.5
			    * (y[0][k] + y[0][k - 1]));
