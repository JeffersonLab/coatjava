 ...
 * @author bondi
 */
public final class LDRDGeant4Factory extends Geant4Factory {
    
    private int nRegions  = LDRDConstants.NREGIONS;
    private int nSectors  = LDRDConstants.NSECTORS;
    private int nChambers = LDRDConstants.NCHAMBERS;
     ...
     * @param nRegions
     */
    public LDRDGeant4Factory( DatabaseConstantProvider cp, boolean prototype, int nRegions) {
    ...
    public double getChamberThickness(){
        double chamberT =0;
         for (int i=0; i< LDRDConstants.CHAMBERVOLUMESTHICKNESS.length; i++ )
             chamberT+=LDRDConstants.CHAMBERVOLUMESTHICKNESS[i];
         return chamberT;
    }
    ...
    public double[] getSectorDimensions(){
        ...
        if(isProto==false){
            SectorDimensions[0] = (this.getChamberThickness())/2. + LDRDConstants.ZENLARGEMENT ;
            SectorDimensions[1] = LDRDConstants.SECTORHEIGHT/2 + LDRDConstants.YENLARGEMENT ;
            SectorDimensions[2] = LDRDConstants.DX0CHAMBER0 + LDRDConstants.XENLARGEMENT ;
            SectorDimensions[3] = (SectorDimensions[1]*2)*Math.tan(Math.toRadians(LDRDConstants.THOPEN/2))+SectorDimensions[2];  
            SectorDimensions[4] = Math.toRadians(LDRDConstants.THTILT);  
        }else{
            
            Line3D AB = new Line3D(LDRDConstants.Apoint, LDRDConstants.Bpoint);
            Line3D CD = new Line3D(LDRDConstants.Cpoint, LDRDConstants.Dpoint);
            
            double DX0_PROTO = LDRDConstants.Cpoint.distance(LDRDConstants.Dpoint);
            double DX1_PROTO = LDRDConstants.Apoint.distance(LDRDConstants.Bpoint);
            ...
            SectorDimensions[0] = (this.getChamberThickness())/2. + LDRDConstants.ZENLARGEMENT ;
            SectorDimensions[1] = h_proto/2 + LDRDConstants.YENLARGEMENT ;
            SectorDimensions[2] = DX0_PROTO/2 + LDRDConstants.XENLARGEMENT ;
            SectorDimensions[3] = DX1_PROTO/2 + LDRDConstants.XENLARGEMENT ;
            SectorDimensions[4] = Math.toRadians(LDRDConstants.THTILT);  
    ...
    */
    ...
    public Vector3d getCenterCoordinate(int isector, int iregion)
    {
        ...
        }else
        {
            ...
            vCenter.x = 0 ;
            vCenter.y = LDRDConstants.SECTORHEIGHT/2*Math.cos(Math.toRadians(LDRDConstants.THTILT))+LDRDConstants.YMIN[iregion];
            vCenter.z =-LDRDConstants.SECTORHEIGHT/2*Math.sin(Math.toRadians(LDRDConstants.THTILT))+LDRDConstants.ZMIN[iregion];
       ...
       */
        ...
        */
        ...
        for (int ich = 0; ich < Nchambers; ich++) {
            ...
            double y_chamber = (2*ich+1)*(LDRDConstants.SECTORHEIGHT/LDRDConstants.NCHAMBERS/2+0.05);
            ...
            if(isProto==false) chamberVolume.translate(0.0,y_chamber-LDRDConstants.SECTORHEIGHT/2,0. );
    ...
    public Geant4Basic createChamber(int iSector, int iRegion, int iChamber) {
        ...
        for (int i=0; i< LDRDConstants.CHAMBERVOLUMESTHICKNESS.length; i++ ){
            ...
            if(i==0) {daughterVolumeZ = LDRDConstants.CHAMBERVOLUMESTHICKNESS[i]/2 - (this.getChamberThickness())/2.;
             } else daughterVolumeZ += LDRDConstants.CHAMBERVOLUMESTHICKNESS[i-1]/2 + LDRDConstants.CHAMBERVOLUMESTHICKNESS[i]/2;
            
            daughterVolumeY = -daughterVolumeZ *Math.tan(Math.toRadians(LDRDConstants.THTILT));
          
            
            Geant4Basic daughterVolume = new G4Trap("daughter_volume",
                LDRDConstants.CHAMBERVOLUMESTHICKNESS[i]/2, -chamberThilt, Math.toRadians(90.0),
                daughterDY, daughterDX0, daughterDX1, 0.0,
                daughterDY, daughterDX0, daughterDX1, 0.0);
            
            if (isProto==true) {
                daughterVolume.setName("rg" + (iRegion + 1) + "_s" + (iSector + 1) + "_c" + (iChamber +1) +"_"+LDRDConstants.CHAMBERVOLUMESNAME[i] + "_proto");
            }else{
                daughterVolume.setName("rg" + (iRegion + 1) + "_s" + (iSector + 1) + "_c" + (iChamber +1) +"_"+LDRDConstants.CHAMBERVOLUMESNAME[i] );
            }
    ...
    public double[] getChamber_Dimensions(int ichamber)
    {
        ...
        if(isProto == false){  
            chamber_Dimensions[0] = (this.getChamberThickness())/2. + LDRDConstants.ZENLARGEMENT/2;
            chamber_Dimensions[1] = LDRDConstants.SECTORHEIGHT/LDRDConstants.NCHAMBERS/2+0.05;
            chamber_Dimensions[2] = (ichamber*LDRDConstants.SECTORHEIGHT/LDRDConstants.NCHAMBERS)*
                                        Math.tan(Math.toRadians(LDRDConstants.THOPEN/2.))+ LDRDConstants.DX0CHAMBER0 +0.1; 
            chamber_Dimensions[3] = (LDRDConstants.SECTORHEIGHT/LDRDConstants.NCHAMBERS)*
                                        Math.tan(Math.toRadians(LDRDConstants.THOPEN/2.))+chamber_Dimensions[2];  
            chamber_Dimensions[4] = Math.toRadians(LDRDConstants.THTILT);
        }else
        {
            Line3D AB = new Line3D(LDRDConstants.Apoint, LDRDConstants.Bpoint);
            Line3D CD = new Line3D(LDRDConstants.Cpoint, LDRDConstants.Dpoint);
            ...
            chamber_Dimensions[0] = (this.getChamberThickness())/2. + LDRDConstants.ZENLARGEMENT/2. ;
            chamber_Dimensions[1] = h_proto/2 + LDRDConstants.YENLARGEMENT -0.1 ;
            chamber_Dimensions[2] = DX0_PROTO/2 + LDRDConstants.XENLARGEMENT -0.1 ;
            chamber_Dimensions[3] = DX1_PROTO/2 + LDRDConstants.XENLARGEMENT -0.1 ; 
            chamber_Dimensions[4]=Math.toRadians(LDRDConstants.THTILT);
    ...
    public double[] getChamber_daughter_Dimensions(int ichamber){
        ...
        if(isProto == false)
        {
            chamber_daughter_Dimensions[0] = LDRDConstants.SECTORHEIGHT/LDRDConstants.NCHAMBERS/2 ;
            chamber_daughter_Dimensions[1] = (ichamber*LDRDConstants.SECTORHEIGHT/LDRDConstants.NCHAMBERS)
                             * Math.tan(Math.toRadians(LDRDConstants.THOPEN/2.))
                             + LDRDConstants.DX0CHAMBER0 ;
    
            chamber_daughter_Dimensions[2] = (LDRDConstants.SECTORHEIGHT/LDRDConstants.NCHAMBERS)
                             * Math.tan(Math.toRadians(LDRDConstants.THOPEN/2.))+chamber_daughter_Dimensions[1];
    ...
    public static void main(String[] args) {
        ...
        LDRDGeant4Factory factory = new LDRDGeant4Factory(cp, false, 2);
